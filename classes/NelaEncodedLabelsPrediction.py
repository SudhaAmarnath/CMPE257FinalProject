...
class NelaEncodedLabelsPrediction():
    ...
    def __init__(self):
        ...
        self.nb_pipeline_ngram.fit(dataTrain['name'], dataTrain['encoded_label'])
        predicted_nb_ngram = self.nb_pipeline_ngram.predict(dataTest['name'])
        accscore = metrics.accuracy_score(dataTest['encoded_label'], predicted_nb_ngram)
        print("Nela Encoded Labels Feature Prediction - accuracy:   %0.6f" % accscore)
...
if __name__ == "__main__":
    ...
    basedir = pickle.load(open('./models/basedir.pkl', 'rb'))
    nelaencodedlabels_filename_pkl = basedir + 'models/nelaencodedlabels_feature_av4.pkl'
    print(nelaencodedlabels_filename_pkl)
    nen = NelaEncodedLabelsPrediction()
    text1 = dataTrain['name'][0]
    text2 = dataTrain['name'][1]
    print(nen.predict(text1), text1)
    print(nen.predict(text2), text2)
    pickle.dump(nen, open(nelaencodedlabels_filename_pkl, 'wb'))
    del nen
